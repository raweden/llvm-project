...
bool FixFunctionBitcasts::runOnModule(Module &M) {
  ...
  for (Function &F : M) {
    ...
    if (F.getCallingConv() == CallingConv::Swift) {
      continue;
    } else if (F.getName().startswith("objc_msgSend") || F.getName().startswith("method_invoke")) {
      continue; // methods generated by the WebAssemblyGenObjCMsgSendFuncs needs to be ignored.
    }
    findUses(&F, F, Uses);
